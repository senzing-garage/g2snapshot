...
def setup_entity_queue_api_single(thread_id, threadStop, entity_queue, resume_queue):
    ...
    try: 
        g2Engine = G2Engine()
        g2Engine.initV2('G2Snapshot%s' % thread_id, iniParams, False)
    except G2Exception as ex:
        printWithNewLines(f'G2Exception: {ex}', 'B')
        with shutDown.get_lock():
            shutDown.value = 1
...
if __name__ == '__main__':
    ...
    threadCount = int(os.getenv('SENZING_THREAD_COUNT')) if os.getenv('SENZING_THREAD_COUNT', None) and os.getenv('SENZING_THREAD_COUNT').isdigit() else 8
    ...
    try:
        ...
        threadCount = math.ceil(physical_cores * calc_cores_factor)
